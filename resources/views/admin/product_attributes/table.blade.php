...
<tr>
    <td><a target="_blank" href="{{ url ('product/'.$row->seo_url ) }}">{{ $row->product_code }}</a></td>
	<td><a target="_blank" href="{{ url ('product/'.$row->category_url ) }}">{{ $row->category_name }}</a></td>
    <td>{{ $row->sku }}</td>
    <td>{{ $row->size }}</td>
     <td>{{ $row->price }}</td>
    <td>
        <div class="input-group stock-group" data-id="{{ $row->id }}">
            <button  class="btn btn-outline-secondary btn-sm decrease-stock" type="button">-</button>
            <input type="text" class="form-control form-control-sm stock-input text-center @if($row->stock < 5) low-stock @endif" value="{{ $row->stock }}">
            <button  class="btn btn-outline-secondary btn-sm increase-stock" type="button">+</button>
        </div>
		
		
    </td>
   <td>
                <a href="javascript:;"  class="stock_logs" data-id="{{ $row->id }}">
				<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000" version="1.1" id="Capa_1" width="20px" height="20px" viewBox="0 0 548.291 548.291" xml:space="preserve">
				<g>
					...
					<path d="M212.533,305.37c0,28.535,13.407,48.64,35.452,48.64c22.268,0,35.021-21.186,35.021-49.5   c0-26.153-12.539-48.655-35.237-48.655C225.504,255.854,212.533,277.047,212.533,305.37z"/>
				</g>
				</svg>
               <a>
   </td>
...
<tr>
    <td colspan="5">
		...
		{{ $products->links('vendor.pagination.bootstrap-4') }} <!-- pagination -->
    </td>
	<td  colspan="2" class="text-center" >
	 <a href="{{ route('product.attributes.export') }}" class="stock-export-btn "> Export </a>
	</td>
</tr>
@endif
@endforeach