                      ...
                      <p class="form-row">
                        ...
                        <div class="row align-items-center row-lost">
                          <div class="col-lg-4 col-6">
                              <button type="submit" class="btn-cart2 r-btn" name="login" value="Log in">Submit</button>
                          </div>
...
function registerform(){ 
	...
	grecaptcha.execute("{{ env('RECAPTCHA_SITE_KEY') }}")
		.then(function(token) {
			...
			$.ajax({
				url: "{{ url('register') }}", // form action URL
				type: "POST",                 // method
				data: $('#RegisterForm').serialize(), // serialize form data including g-recaptcha-response
				success: function(data) {
